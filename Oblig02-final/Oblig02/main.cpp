...
int main()
{
	...
	worker_threads.start(); // Creates 4 internal threads
	event_loop.start(); // Creates 1 internal thread

	//Posting tasks
	/*
	for (int i = 0; i < 3; i++) worker_threads.post_timeout([] { worker_greeting(this_thread::get_id()); }, 1000);

	for (int i = 0; i < 3; i++) worker_threads.post([] { worker_greeting(this_thread::get_id()); });

	for (int i = 0; i < 3; i++) event_loop.post_timeout([] { event_greeting(this_thread::get_id()); }, 1000);

	for (int i = 0; i < 3; i++) event_loop.post([] { event_greeting(this_thread::get_id()); });
	*/
